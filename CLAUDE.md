...
**LLM Enhancement (v4.0–4.1)** — Optional LLM-powered emoji enhancement layer. Dual-format: `enhanceManPage()` generates both `emoji_html` (HTML-direct, for default view) and `emoji_md` (Markdown, for /markdown format) via 2 LLM calls. `callLLM()` handles the API call with 300s timeout and `finish_reason: "length"` truncation detection. `cleanEmojiHtml()` post-processes LLM output: strips DOCTYPE/html/head/body wrappers, removes `<script>`/`<style>`/`<meta>`/`<link>`/`<title>` with content, then `strip_tags()` with safe allowlist. Enhanced HTML is the default view when `emoji_html` cache exists; `?format=html` bypasses.
