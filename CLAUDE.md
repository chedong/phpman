...
- **`<!DOCTYPE html>` standards mode** — simple HTML5 doctype (no DTD URL) avoids Limited Quirks Mode. No HTML5 semantic tags (`<nav>`, `<section>`, `<article>`), no `og:` meta tags. Use `<div id="...">` and `<p>` instead.
