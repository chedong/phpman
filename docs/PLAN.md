...
```
v2.1 (2026.05 已发布)    v3.0 (2026.Q3)           v4.0 (2026.Q4+)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
man / perldoc / info  →  多语言参考扩展      →    AI 智能文档生成
MCP Server (JSON-RPC)     pydoc3 / ri              智能翻译 (保留标识符)
Markdown 输出              索引侧栏导航              Cheat Sheet 模式
JSON API (语义化)          搜索增强                  参数样例自动生成
TLDR 端点                  MCP 流式/分页             本地缓存 (永久)
安全加固                   代码拆分                  本地全文搜索
跨平台宽度控制             模糊搜索                  国际化 (LANG+AI)
                          可观测性                   
                                                     
v2.1 = 当前生产版本 | v3.0 = 多语言 + 搜索 | v4.0 = AI 重塑文档体验
```
...
```
┌──────────────────────────────────────────────────────────────┐
│ v3.0 — 多语言参考扩展 (2026.Q3)                              │
├──────────────────────────────────────────────────────────────┤
│ pydoc3 / ri 支持        索引侧栏导航 (>150 条)               │
│ 搜索增强 (模糊搜索)      MCP 流式/分页输出                    │
│ MCP 错误响应标准化         MCP 动态工具发现                   │
│ 代码架构拆分 (src/)       可观测性 (SQLite 统计)              │
│ Go doc / Rustdoc (远期)                                      │
└──────────────────────────────────────────────────────────────┘
...
**v3.0 (1-2 月):** pydoc/ri 扩展 + 索引侧栏导航 + 搜索增强 + MCP 优化 + 代码拆分
**v4.0 (3-6 月):** AI 翻译 + Cheat Sheet + 参数样例 + 本地缓存 + 本地全文搜索 + 国际化
